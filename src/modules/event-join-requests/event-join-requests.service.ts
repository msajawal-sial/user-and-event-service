...
export class EventJoinRequestsService {
  ...
  async createEventJoinRequest(
    eventJoinRequest: CreateEventJoinRequestDto,
    userId: number,
    ...
    try {
      ...
      if (error?.code === PostgresErrorCode.UniqueViolation) {
        throw new HttpException(
          'Event join request already exists',
    ...
    const createdEventJoinRequest = await this.getEventJoinRequest(
      newEventJoinRequest.id,
    );
    ...
    this.emailService.sendEmail(
      ...
      {
        user: createdEventJoinRequest.user.name,
        event: createdEventJoinRequest.event.title,
      },
    );
  ...
  async updateEventJoinRequest(
    id: number,
    updateEventJoinRequest: UpdateEventJoinRequestDto,
  ): Promise<boolean> {
    const result = await this.eventJoinRequestsRepository.update(
      id,
      updateEventJoinRequest,
    );
